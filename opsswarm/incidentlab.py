...
def start_demo(scenario_id):
    ...
    with LOCK:
        _inject_service(SCENARIOS[scenario_id]); run=_new_run(scenario_id); _workflow(run)
        services=_services(); _evidence(run,"S8","opsswarm-incident-manager","incident context",{"scenario":scenario_id,"services":services})
        _evidence(run,"S5","opsswarm-incident-manager","evidence aggregation",{"metrics":services,"dependencies":_deps(services)})
        if run["policy_decision"]=="AUTO":
            run["stages"]["Recovery"]["state"]="RUNNING"; run["recovery_state"]="RUNNING"
            result=_apply_recovery(run); _evidence(run,"Recovery","opsswarm-recovery-responder","execute authorized recovery",result)
            run["stages"]["Recovery"]["state"]="COMPLETED"; run["stages"]["S6"]["state"]="COMPLETED"; _verify(run)
        else:
            run["stages"]["Recovery"]["state"]="WAITING"
        _write(run); return run
...
def demo_approve(req: RecoveryRequest):
    r=latest()
    if not r: raise HTTPException(404,"no active run")
    return approve(r,req.option_id)
...
async function approve(){if(!last)return;render(await j('/api/demo/approve',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({option_id:last.recovery_options[0].id})}))}
